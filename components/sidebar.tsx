...
export function Sidebar({ open, onOpenChange }: SidebarProps) {
  ...
  const isCmsRoute = pathname.startsWith("/dashboard/cms")
  const [cmsExpanded, setCmsExpanded] = useState(false)
  const [user, setUser] = useState<User | null>(null)

  // Always expand CMS dropdown when on CMS route
  useEffect(() => {
    if (isCmsRoute) setCmsExpanded(true)
  }, [isCmsRoute])

  // Redirect /dashboard/cms → /dashboard/cms/about
  useEffect(() => {
    if (pathname === "/dashboard/cms") {
      router.replace("/dashboard/cms/about")
    }
  }, [pathname, router])
  ...
  const handleCmsClick = () => {
    // Always expand CMS and go to About Us (default)
    setCmsExpanded(true)
      ...
      >
        <div className="flex h-full flex-col">
          ...
          <nav className="flex-1 p-4 space-y-2">
            ...
            <div>
              <Button
                variant={isCmsRoute ? "default" : "ghost"}
                className={cn(
                  "w-full justify-between text-sidebar-foreground hover:bg-sidebar-accent/50 h-11",
                  isCmsRoute ? "bg-[#ff5a5f] text-white" : ""
                )}
                onClick={handleCmsClick}
                aria-expanded={cmsExpanded}
              >
